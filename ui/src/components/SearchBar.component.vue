<template>
  <el-row :offset="1" :gutter="40" :align="'center'" class="flex flex-row pt-4 pb-2 bg-[#F9F0C3] border-amber-600 border-b">
    <el-col :xs="24" :sm="9" :md="8" :lg="6" :xl="4" class="px-0 pb-4 max-w-0 h-auto text-white">
      <el-row :justify="'center'" :align="'middle'" class="px-2">
        <p class="font-light" v-html="siteName"></p>
      </el-row>
    </el-col>
    <el-col :xs="24" :sm="15" :md="16" :lg="18" :xl="20" class="pt-8 px-4 pr-4 max-w-0 h-auto">
      <el-row :justify="'center'" :gutter="20" :align="'middle'">
        <label for="searchInput" class="">
          <input @keyup.enter="searchInputField" type="text" class="px-4 py-2 w-80 border text-black rounded"
                 placeholder="Search..."
                 v-model="searchQuery"
                 v-on:change="searchInputField"
                 name="searchInput" id="searchInput" ref="searchInput"/>
          <button tabindex="-1"
                  class="border-r cursor-pointer outline-none focus:outline-none transition-all text-gray-500 hover:text-red-600">
